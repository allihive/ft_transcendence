...
const matchmaking_repository_1 = require("../database/repositories/matchmaking.repository");
...
class MatchmakingService {
    constructor(config = DEFAULT_CONFIG) {
        this.matchMakingInterval = null; // ADD this one
        this.config = config;
    ...
    */
    ...
    updatePlayerStatus(playerId, status) {
      ...
      `;
            ...
            index_1.default.run(query, [status, updatedAt, playerId], (err) => {
                if (err) {
                    reject(err);
                }
                else {
    ...
    async findMatches() {
        try {
            const onlinePlayers = await (0, matchmaking_repository_1.findOnlinePlayers)();
            ...
            const availablePlayers = onlinePlayers.filter(player => this.matchMakingQueue.has(player.playerId));
    ...
    createMatches(players) {
        const matches = [];
        const usedPlayers = new Set(); // Track players already matched
        ...
        for (let i = 0; i < sortedPlayers.length - 1; i++) {
            ...
            if (usedPlayers.has(player1.playerId))
                continue;
            ...
            for (let j = i + 1; j < sortedPlayers.length; j++) {
                ...
                if (usedPlayers.has(player2.playerId))
                    continue;
    ...
    /**
       ...
       */
    async completeMatch(matchId, winnerId, winnerScore, loserScore) {
        try {
            ...
            const { newWinnerScore, newLoserScore } = this.calculateNewScores(winner, loser, winnerScore, loserScore);
            // Update both players' ratings in the database
            await this.updatePlayerScore(winnerId, newWinnerScore);
            await this.updatePlayerScore(winnerId === match.player1.playerId ? match.player2.playerId : match.player1.playerId, newLoserScore);
    ...
    */
    calculateNewScores(winner, loser, winnerGameScore, loserGameScore) {
   ...
   */
    updatePlayerScore(playerId, newScore) {
      ...
      `;
            const updatedAt = new Date().toISOString();
            index_1.default.run(query, [newScore, updatedAt, playerId], (err) => {
                if (err) {
                    reject(err);
                }
                else {
    ...
    async getPlayerQueueStatus(playerId) {
